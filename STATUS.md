...
Last touched: 2026-05-30 (Wave-7: Cypro-Minoan Type-II availability gap, no public corpus).
...
| Cross-substrate numeric agreement, Cypro-Minoan | `not-attempted` (Type-II: no public corpus) | PR-X availability sweep 2026-05-30 (register id `exp-1780158016-064477022`, REFUTED). GitHub returns only `notofonts/cypro-minoan` (Noto font, no inscriptions) and `Lackadaisical-Security/Cypro-Minoan-Research` (CC-BY-NC self-asserted lexicon, not a transcription corpus). Wikidata SPARQL and archive.org return zero machine-readable scholarly-attested CM transcriptions. Olivier 2007 HoChyMin and Ferrara 2012-2013 are print-only De Boccard / Oxford. Per task honesty caveat, no synthetic corpus fabricated; script skipped at the corpus-availability layer. Zig substrate is unchanged: `audit-residual --json PATH` would accept a CM corpus if one becomes public under an AGPL-compatible license. See `~/symbols/PREREGISTRATION_PR_X.md`. |
...
- **Cypro-Minoan parity attempt 2026-05-30 hit a Type-II availability gap.** No public AGPL-compatible machine-readable scholarly CM transcription corpus exists; the audit harness is unchanged and the BAKEOFF row is `not-attempted` rather than `audited` or `posture-observable (pending)`. See `~/symbols/PREREGISTRATION_PR_X.md` and register id `exp-1780158016-064477022` (REFUTED, Type-II catch).
...
- **2026-05-30: Cypro-Minoan availability sweep (Type-II catch).** PR-X (`exp-1780158016-064477022`) registered before any data fetch as the availability-test hypothesis for extending the F7 PR-A parity gate to Cypro-Minoan. Verdict REFUTED, Type-II = 1 on the experiment register. Four substrates queried (GitHub repo search, Wikidata SPARQL, archive.org, Unicode block testdata); no source returned a machine-readable, AGPL-compatible, scholarly-attested CM transcription corpus. The only public CM data are a Noto font and a CC-BY-NC self-asserted lexicon (with a "first successful decipherment" framing that fails field consensus); Olivier 2007 HoChyMin and Ferrara 2012-2013 are print-only De Boccard / Oxford volumes. Per task honesty caveat, no synthetic CM corpus was fabricated. Zig substrate unchanged: the `audit-residual --json PATH` subcommand already accepts arbitrary Corpus JSON and would drive the parity audit if a public CM corpus is released. See `~/symbols/PREREGISTRATION_PR_X.md` for the full sweep and the conditional H_PR_X_2 that would unlock the row.
